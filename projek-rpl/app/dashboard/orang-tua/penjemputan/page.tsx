...
return (
  <div className="space-y-8" style={{ fontFamily: "'Montserrat', sans-serif" }}>
    ...
    {children.length > 1 && (
      <div>
        <p className="text-xs font-bold text-[#4A4A4A] uppercase tracking-wider mb-3 ml-1">
          Pilih Anak
        </p>
        ...
        {confirmed && todayRequest && (
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-[#4A4A4A] mb-1">
                  Status Saat Ini
                </p>
            ...
            <div className="mt-6 w-full">
              <div className="flex items-center">
                <div className="w-7 h-7 rounded-full bg-[#1883FF] text-white flex items-center justify-center text-[11px] font-bold shadow-sm shadow-[#1883FF]/30 shrink-0">
                  ✓
                </div>

                <div
                  className={`flex-1 h-1.5 mx-2 rounded-full transition-all duration-700 ${
                    isApproved ? "bg-[#1883FF]" : "bg-[#E5E7EB]"
                  }`}
                />

                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold shadow-sm shrink-0 transition-all
                    ${
                      isApproved
                        ? "bg-[#1883FF] text-white shadow-[#1883FF]/30"
            ...
            {todayRequest.foto_url && (
              <div className="mt-6">
                <p className="mb-2 text-sm font-bold text-[#4A4A4A] text-center">
                  Foto Bukti Penjemputan
                </p>

                <div className="flex justify-center">
                  <img
                    src={todayRequest.foto_url}
                    alt="Pickup proof"
                    className="max-h-80 max-w-full rounded-2xl object-contain border bg-white"
                  />
                </div>
              </div>
            )}
          ...
          <div className="bg-white rounded-[2rem] p-6 border border-[#FFE26F]/40 shadow-sm space-y-5">
            ...
            {confirmed ? (
              <div className="space-y-3">
                <div className="bg-[#C4E02F]/15 border border-[#C4E02F]/40 rounded-2xl p-5 text-center space-y-2">
                  ...
                  <p className="font-bold text-[#1A1A1A]">
                    Konfirmasi Terkirim ke Pengasuh!
                  </p>

                  <p className="text-sm text-[#4A4A4A] font-light">
                    <strong>{todayRequest?.pickup_person_name}</strong>{" "}
                    ({RELATIONSHIP_LABEL[todayRequest?.relationship ?? ""] ?? todayRequest?.relationship}) akan menjemput pukul{" "}
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-[#1A1A1A] mb-2 ml-1">
                    Nama penjemput
                  </label>
                ...
                {history.map((h) => (
                  <div key={h.id} className="flex items-center gap-3 p-3 rounded-2xl bg-[#FFFDF7] border border-[#FFE26F]/30">
                    ...
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      {h.pickup_time && (
                        <span className="text-sm font-black text-[#1883FF]">
                          {formatJam(h.pickup_time)}
                        </span>
                      )}
                      ...
                      <span
                        className={`text-[9px] font-bold px-2 py-0.5 rounded-full
                          ${
                            h.status === "approved"
                              ? "bg-[#C4E02F]/20 text-[#5a8a00]"
                              : "bg-[#FFE26F]/30 text-[#a07000]"
                          }`}
                      >
