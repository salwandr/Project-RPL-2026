...
const STATUS_CONFIG: Record<CheckInStatus, {
  ...
  belum: {
    label: "Belum Check-in", color: "#FEB700", bg: "bg-[#FFE26F]/20", border: "border-[#FFE26F]",
    icon: (
  ...
  hadir: {
    label: "Sudah Hadir", color: "#4a7500", bg: "bg-[#C4E02F]/15", border: "border-[#C4E02F]/40",
    icon: (
  ...
  izin: {
    label: "Izin Tidak Hadir", color: "#3344aa", bg: "bg-[#99ADFF]/15", border: "border-[#99ADFF]/30",
    icon: (
  ...
  sakit: {
    label: "Sakit", color: "#aa3366", bg: "bg-[#FFA9DD]/15", border: "border-[#FFA9DD]/30",
    icon: (
...
function getAvatarBg(kelas: string) {
  return avatarBg[kelas] ?? "#99ADFF";
}
...
export default function PengasuhKedatanganPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const { data: attData, error: attErr } = await supabase
        .from("attendance")
        .select("id, child_id, status, jam_checkin, keterangan")
        .eq("date", today);
      if (attErr) throw attErr;

      const attMap = new Map(attData?.map((a) => [a.child_id, a]) ?? []);

      const mapped: Child[] = (kidsData ?? []).map((k) => {
        ...
        // Tentukan kelas dari program — sesuaikan kalau nama program di DB berbeda
        const kelasMap: Record<string, string> = {
          full_day: "Rainbow Room",
          half_day: "Sunshine Class",
          playgroup: "Star Class",
        };
        const kelas = kelasMap[k.program] ?? k.program ?? "Rainbow Room";
        ...
        return {
          ...
          programJam: k.program === "full_day" ? "07:00–17:00"
                    : k.program === "half_day" ? "07:00–12:00"
                    : "08:00–11:00",
          status: (att?.status as CheckInStatus) ?? "belum",
          jamCheckin: att?.jam_checkin
            ? att.jam_checkin.slice(0, 5)   // "07:15:00" → "07:15"
            : null,
      ...
      setSelectedChild((prev) => {
        if (!prev) return mapped.find((c) => c.status === "belum") ?? mapped[0] ?? null;
  ...
  const handleCheckIn = async () => {
    ...
    try {
      const today   = todayStr();
      const jamNow  = new Date().toTimeString().slice(0, 8); // "HH:MM:SS"

      if (child.attendanceId) {
        // Update existing row
        const { error } = await supabase
          .from("attendance")
          .update({ status: "hadir", jam_checkin: jamNow, keterangan: null })
          .eq("id", child.attendanceId);
        if (error) throw error;
      } else {
        // Insert new row
        const { error } = await supabase
          .from("attendance")
          .insert({ child_id: child.id, date: today, status: "hadir", jam_checkin: jamNow, keterangan: null });
        if (error) throw error;
      }
      await loadData();
    } catch (e: any) {
  ...
  const handleAbsen = async () => {
    ...
    try {
      ...
      if (child.attendanceId) {
        const { error } = await supabase
          .from("attendance")
          .update({ status: jenisAbsen, jam_checkin: null, keterangan: alasanAbsen || null })
          .eq("id", child.attendanceId);
        if (error) throw error;
      } else {
        const { error } = await supabase
          .from("attendance")
          .insert({ child_id: child.id, date: today, status: jenisAbsen, jam_checkin: null, keterangan: alasanAbsen || null });
        if (error) throw error;
      }
      setAlasanAbsen("");
      setJenisAbsen("izin");
      ...
      setError(e?.message ?? "Gagal menyimpan ketidakhadiran.");
    }
  };
  ...
  const focusCfg   = focusChild ? STATUS_CONFIG[focusChild.status] : STATUS_CONFIG.belum;
  ...
  if (error) {
    return (
      <div className="space-y-4">
        ...
        <button onClick={loadData} className="px-4 py-2 bg-[#1883FF] text-white rounded-xl text-sm font-bold">
          Coba Lagi
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── MODAL CHECK-IN ── */}
      {modal?.type === "checkin" && (
        <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(0,0,0,0.45)" }} onClick={() => setModal(null)}>
          <div className="bg-white w-full max-w-md rounded-t-3xl p-6 pb-9" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-4 mb-5">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-base font-bold shrink-0"
                style={{ background: getAvatarBg(modal.child.kelas) + "20", color: getAvatarBg(modal.child.kelas) }}>
                {modal.child.avatar}
              </div>
              <div>
            ...
            <div className="flex items-center gap-3 bg-[#C4E02F]/10 border border-[#C4E02F]/40 rounded-2xl p-4 mb-5">
              ...
              <p className="text-[12px] text-[#4a7500] font-semibold leading-snug">Waktu check-in akan dicatat otomatis sesuai jam sekarang</p>
            </div>
            <div className="flex justify-between items-center bg-[#F4F6FA] rounded-xl px-4 py-3 mb-6">
              ...
              <span className="text-[18px] font-bold text-[#1A1A1A]">{nowTimeStr()} WIB</span>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setModal(null)} className="flex-1 py-4 border border-[#E8E4DB] rounded-2xl text-[13px] font-bold text-[#4A4A4A] hover:bg-[#F4F6FA] transition-colors">Batal</button>
              <button onClick={handleCheckIn} disabled={loadingId === modal.child.id}
          ...
          <div className="bg-white w-full max-w-md rounded-t-3xl p-6 pb-9" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mb-4">
              {(["izin", "sakit"] as const).map((j) => (
                <button key={j} onClick={() => setJenisAbsen(j)}
            ...
            <textarea value={alasanAbsen} onChange={(e) => setAlasanAbsen(e.target.value)}
      ...
      {focusChild && (
        <div className={`${focusCfg.bg} border-2 ${focusCfg.border} rounded-[2rem] p-6`}>
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm shrink-0">{focusCfg.icon}</div>
              ...
              return (
                <button key={child.id} onClick={() => setSelectedChild(child)}
                  className={`w-full flex items-center gap-3 p-3 rounded-2xl border-2 text-left transition-all hover:scale-[1.01] active:scale-[0.99]
                    ${isSelected ? "border-[#1883FF] bg-[#1883FF]/5" : "border-[#FFE26F]/40 bg-white hover:border-[#1883FF]/30"}`}>
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center text-[10px] font-bold shrink-0"
                    style={{ background: getAvatarBg(child.kelas) + "20", color: getAvatarBg(child.kelas) }}>
                    {child.avatar}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="shrink-0 text-right">
                    ...
                    {child.keterangan && <p className="text-[9px] text-[#4A4A4A] mt-0.5">{child.keterangan}</p>}
                  </div>
                </button>
              );
            })}
          ...
          <div className="bg-white rounded-[2rem] p-6 border border-[#FFE26F]/40 shadow-sm space-y-5">
            ...
            <div className={`${focusCfg.bg} border ${focusCfg.border} rounded-2xl p-4 flex items-center gap-4`}>
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-bold shrink-0"
                style={{ background: getAvatarBg(focusChild.kelas) + "30", color: getAvatarBg(focusChild.kelas) }}>
                {focusChild.avatar}
              </div>
              <div className="flex-1 min-w-0">
            ...
            {focusChild.status === "hadir" ? (
              <div className="bg-[#C4E02F]/15 border border-[#C4E02F]/40 rounded-2xl p-5 text-center space-y-2">
                ...
                <p className="text-sm text-[#4A4A4A] font-light">
                  {focusChild.name} tercatat hadir pukul <strong className="text-[#1A1A1A]">{focusChild.jamCheckin} WIB</strong>
                </p>
              </div>
              ...
              <div className="rounded-2xl p-5 text-center space-y-3"
                ...
                <p className="font-bold text-[#1A1A1A]">{focusChild.status === "izin" ? "Izin Tidak Hadir" : "Sakit"}</p>
                {focusChild.keterangan && <p className="text-sm text-[#4A4A4A] font-light">{focusChild.keterangan}</p>}
                <button onClick={() => setModal({ type: "checkin", child: focusChild })}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => { setJenisAbsen("izin"); setModal({ type: "absen", child: focusChild }); }}
                  ...
                  <button onClick={() => { setJenisAbsen("sakit"); setModal({ type: "absen", child: focusChild }); }}
                ...
                <p className="text-[11px] text-[#4A4A4A]/60 text-center font-light">Atau tap nama anak lain di daftar untuk beralih</p>
              </div>
            )}
            ...
            <div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {children.filter((c) => c.status === "hadir").length === 0 ? (
                  ...
                  children.filter((c) => c.status === "hadir").map((c) => (
