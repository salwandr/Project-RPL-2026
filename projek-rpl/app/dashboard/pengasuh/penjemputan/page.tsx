...
const initialChildren: Child[] = [
  { id: 1, name: "Anak 1", kelas: "Rainbow Room",   jamMasuk: "07:10", penjemput: "Ayah",  status: "dijemput", jamJemput: "15:30", avatar: "A1" },
  { id: 2, name: "Anak 2", kelas: "Sunshine Class", jamMasuk: "07:25", penjemput: "Ibu",   status: "dijemput", jamJemput: "15:45", avatar: "A2" },
  { id: 3, name: "Anak 3", kelas: "Rainbow Room",   jamMasuk: "07:30", penjemput: "Ayah",  status: "menunggu", jamJemput: null,    avatar: "A3" },
  { id: 4, name: "Anak 4", kelas: "Star Class",     jamMasuk: "07:15", penjemput: "Ibu",   status: "menunggu", jamJemput: null,    avatar: "A4" },
  { id: 5, name: "Anak 5", kelas: "Sunshine Class", jamMasuk: "07:40", penjemput: "Nenek", status: "dijemput", jamJemput: "16:00", avatar: "A5" },
  { id: 6, name: "Anak 6", kelas: "Rainbow Room",   jamMasuk: "07:20", penjemput: "Ayah",  status: "menunggu", jamJemput: null,    avatar: "A6" },
  { id: 7, name: "Anak 7", kelas: "Star Class",     jamMasuk: "07:35", penjemput: "Ibu",   status: "menunggu", jamJemput: null,    avatar: "A7" },
  { id: 8, name: "Anak 8", kelas: "Sunshine Class", jamMasuk: "07:22", penjemput: "Kakek", status: "dijemput", jamJemput: "15:55", avatar: "A8" },
];
...
export default function PengasuhPenjemputanPage() {
  ...
  const handleCheckout = (id: number) => {
    ...
    setChildren((prev) =>
      prev.map((c) =>
        c.id === id ? { ...c, status: "dijemput" as PickupStatus, jamJemput: jam } : c
      )
    );
  };
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[12px] font-semibold text-stone-700">Progress Penjemputan</p>
          ...
          {filtered.map((child) => (
            ...
            >
              ...
              <div className="text-[10px] text-stone-400 hidden sm:block text-right">
                <p>Masuk <span className="font-semibold text-stone-600">{child.jamMasuk}</span></p>
                <p>Dijemput <span className="font-semibold text-stone-600">{child.penjemput}</span></p>
              </div>

              {child.status === "dijemput" ? (
                <div className="text-right flex-shrink-0">
                  <span className="text-[10px] font-bold text-blue-700 bg-pastel-blue/40 px-3 py-1.5 rounded-full shadow-sm block">
                    Dijemput ✓
                  </span>
                  {child.jamJemput && (
                    <p className="text-[9px] text-stone-400 mt-1 text-center">{child.jamJemput}</p>
                  )}
                </div>
              ) : (
                <button
                  onClick={() => handleCheckout(child.id)}
                  className="text-[10px] font-bold text-blue-700 bg-pastel-blue/40 px-3 py-1.5 rounded-full shadow-sm hover:shadow-md hover:scale-105 active:scale-95 transition-all duration-150 flex-shrink-0"
                >
                  Check-out
                </button>
              )}
